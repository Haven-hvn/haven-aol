...
// ═══════════════════════════════════════════════════════════════════
// TC-8: Merkle Attestation Vector Parity (OFFLINE)
// ═══════════════════════════════════════════════════════════════════
//
// Locks in the canonical Merkle attestation algorithm shipped in
// src/backend/main.mo (v2 batchAttestHolding). Each fixture in
// tests/fixtures/merkle-attest-vector-n{1,2,4,20}.json was produced by
// tests/fixtures/generate_merkle_vector.py, which mirrors the Motoko
...
//
// This block doubles as the reference verifier the haven-dapp
// `verifyMerkleAttestation` will adopt in Phase 3.

describe("TC-8: Merkle Attestation Vector Parity", () => {
  const LEAF_PREFIX = Buffer.from([0x00]);
  const NODE_PREFIX = Buffer.from([0x01]);
  const ZERO_LEAF = sha256(Buffer.concat([LEAF_PREFIX, Buffer.from("HAVEN_MERKLE_ZERO", "utf-8")]));
  ...
  function hashLeaf(preimage) {
    return sha256(Buffer.concat([LEAF_PREFIX, preimage]));
  }
  ...
  function leafPreimage(input, cidHash, timestamp, balance) {
    const text =
      `HAVEN_ATTEST_V1:${input.chain}:${input.tokenAddress}:${input.threshold}:` +
      `${input.evmAddress}:${cidHash}:${timestamp}:${balance}`;
    return Buffer.from(text, "utf-8");
  }

  function batchPreimage(input, merkleRootHex, cidCount, timestamp, balance) {
    return (
      `HAVEN_BATCH_ATTEST_V1:${input.chain}:${input.tokenAddress}:${input.threshold}:` +
      `${input.evmAddress}:${merkleRootHex}:${cidCount}:${timestamp}:${balance}`
    );
  }
  ...
  function buildHeap(sortedLeafHashes, limit) {
    const total = 2 * limit - 1;
    const nodes = new Array(total);
    for (let i = 0; i < limit; i++) {
      nodes[limit - 1 + i] = i < sortedLeafHashes.length ? sortedLeafHashes[i] : ZERO_LEAF;
    }
    if (limit > 1) {
      for (let p = limit - 2; p >= 0; p--) {
  ...
  function proofForLeaf(nodes, leafHeapIdx) {
    const out = [];
    let p = leafHeapIdx;
    while (p > 0) {
      let siblingIdx, side;
      ...
      out.push({ side, hash: nodes[siblingIdx].toString("hex") });
      p = (p - 1) >>> 1;
    }
    return out;
  }

  function verifyProof(leafHash, proof, expectedRootHex) {
    let h = leafHash;
    for (const step of proof) {
      const sibling = Buffer.from(step.hash, "hex");
      h = step.side === "left" ? hashNode(sibling, h) : hashNode(h, sibling);
    }
    return h.toString("hex") === expectedRootHex;
  }

  function loadFixture(n) {
    const path = resolve(__dirname, `fixtures/merkle-attest-vector-n${n}.json`);
    return JSON.parse(readFileSync(path, "utf-8"));
  }

  function reproduce(fixture) {
    const { input, expected } = fixture;
    const { timestamp, balanceAtCheck } = expected;
    const submission = input.cidHashes_submissionOrder;

    // Sort with submission index, derive submission→sorted mapping.
    const pairs = submission
      .map((cidHash, subIdx) => ({ cidHash, subIdx }))
      .sort((a, b) => (a.cidHash < b.cidHash ? -1 : a.cidHash > b.cidHash ? 1 : 0));
    const submissionToSorted = new Array(submission.length);
    pairs.forEach((p, sortedIdx) => {
      submissionToSorted[p.subIdx] = sortedIdx;
    });

    const sortedLeafHashes = pairs.map((p) =>
      hashLeaf(leafPreimage(input, p.cidHash, timestamp, balanceAtCheck)),
    );

    const limit = nextPow2(submission.length);
    const nodes = buildHeap(sortedLeafHashes, limit);
    const merkleRoot = nodes[0];

    const leaves = submission.map((cidHash, subIdx) => {
      const sortedIdx = submissionToSorted[subIdx];
      const leafHeapIdx = limit - 1 + sortedIdx;
      return { cidHash, merkleProof: proofForLeaf(nodes, leafHeapIdx) };
    });

    return { merkleRoot, leaves, limit };
  }

  for (const n of [1, 2, 4, 20]) {
    describe(`fixture N=${n}`, () => {
      const fixture = loadFixture(n);
      const { input, expected } = fixture;

      it("constants match (LEAF_PREFIX, NODE_PREFIX, zeroLeaf)", () => {
        assert.equal(fixture.constants.LEAF_PREFIX, "0x00");
        assert.equal(fixture.constants.NODE_PREFIX, "0x01");
        assert.equal(ZERO_LEAF.toString("hex"), expected.zeroLeaf);
      });

      it("padLimit equals nextPow2(cidCount)", () => {
        assert.equal(nextPow2(expected.cidCount), expected.padLimit);
      });

      it("merkleRoot is reproducible from inputs alone", () => {
        const { merkleRoot, limit } = reproduce(fixture);
        assert.equal(limit, expected.padLimit, "padLimit mismatch");
        assert.equal(merkleRoot.toString("hex"), expected.merkleRoot, "merkleRoot mismatch");
      });

      it("per-leaf proofs match fixture (submission order)", () => {
        const { leaves } = reproduce(fixture);
        assert.equal(leaves.length, expected.leaves_submissionOrder.length);
        for (let i = 0; i < leaves.length; i++) {
          assert.equal(
            leaves[i].cidHash,
            expected.leaves_submissionOrder[i].cidHash,
            `leaf ${i} cidHash mismatch (submission order)`,
          );
          assert.deepEqual(
            leaves[i].merkleProof,
            expected.leaves_submissionOrder[i].merkleProof,
            `leaf ${i} merkleProof mismatch`,
          );
        }
      });

      it("every stored proof verifies against stored root", () => {
        for (const leaf of expected.leaves_submissionOrder) {
          const leafHash = hashLeaf(
            leafPreimage(input, leaf.cidHash, expected.timestamp, expected.balanceAtCheck),
          );
          assert.ok(
            verifyProof(leafHash, leaf.merkleProof, expected.merkleRoot),
            `proof for ${leaf.cidHash} did not verify`,
          );
        }
      });

      it("batch preimage matches HAVEN_BATCH_ATTEST_V1 format", () => {
        const expectedPreimage = batchPreimage(
          input,
          expected.merkleRoot,
          expected.cidCount,
          expected.timestamp,
          expected.balanceAtCheck,
        );
        assert.equal(expectedPreimage, expected.batchPreimage_utf8);
      });

      if (n > 1) {
        it("tampering with any proof step breaks verification", () => {
          const leaf = expected.leaves_submissionOrder[0];
          const leafHash = hashLeaf(
            leafPreimage(input, leaf.cidHash, expected.timestamp, expected.balanceAtCheck),
          );
          // Flip one bit in the first sibling hash.
          const tampered = leaf.merkleProof.map((s, i) =>
            i === 0
              ? { side: s.side, hash: (BigInt("0x" + s.hash) ^ 1n).toString(16).padStart(64, "0") }
              : s,
          );
          assert.ok(
            !verifyProof(leafHash, tampered, expected.merkleRoot),
            "tampered proof should NOT verify",
          );
        });

        it("flipping a side label breaks verification", () => {
          const leaf = expected.leaves_submissionOrder[0];
          const leafHash = hashLeaf(
            leafPreimage(input, leaf.cidHash, expected.timestamp, expected.balanceAtCheck),
          );
          const flipped = leaf.merkleProof.map((s, i) =>
            i === 0 ? { side: s.side === "left" ? "right" : "left", hash: s.hash } : s,
          );
          assert.ok(
            !verifyProof(leafHash, flipped, expected.merkleRoot),
            "side-flipped proof should NOT verify",
          );
        });
      }
    });
  ...
  it("RFC 6962 domain separation: leaf-vs-node hashes diverge for same payload", () => {
    // Same 64-byte payload hashed as a leaf preimage MUST NOT equal hashing
    // it as a 2-child internal node (32-byte left ‖ 32-byte right).
    // This is the standard second-preimage defense and is what the 0x00/0x01
    // prefixes buy us.
    const left = Buffer.alloc(32, 0xaa);
    const right = Buffer.alloc(32, 0xbb);
    const asNode = hashNode(left, right);
    const asLeaf = hashLeaf(Buffer.concat([left, right]));
    assert.notEqual(asNode.toString("hex"), asLeaf.toString("hex"));
  });
});
